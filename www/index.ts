...
init().then((wasm) => {
  ...
  const world = World.new(WORLD_WIDTH, snakeSpawnIdx, Direction.Right);
  const worldWidth = world.width();

  const gameStatus = document.getElementById("game-status");
  const gameControlBtn = document.getElementById("game-control-btn");
  const pointsStatus = document.getElementById("points");
  ...
  function drawGameStatus() {
    ...
    pointsStatus.textContent = world.points().toString();
  }
